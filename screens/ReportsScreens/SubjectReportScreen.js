...
export default function SubjectReportScreen({route}) {
    ...
    async function showHandler(modeDay) {
        ...
        try {
            const response = await fetchGeneralReport(selectedDate, '', subject, modeDay, true, '', false);
                    ...
                    <View style={styles.datesButtonsContainer}>
                        ...
                        >
                            <Text style={!switchedToDay ? styles.monthOnButtonText : styles.monthOffButtonText}>
                                Month: {selectedDate.toLocaleString('default', { month: 'long' })}      Subject: {subject}
                            </Text>
                        </Pressable>
                    </View>
                }
